...
class LabelNormalisation(LinguisticBase):
    ...
    def extract_linguistic_features(self, in_file_name, out_file_name=None, label_type="state_align", state_number=5, dur_file_name=None):
        if label_type=="phone_align":
            A = self.load_labels_with_phone_alignment(in_file_name, dur_file_name)
        elif label_type=="state_align":
            A = self.load_labels_with_state_alignment(in_file_name,state_number=5)
        else:
...
#            self.question_dict, self.ori_question_dict = self.load_question_set(question_file_name)
    ...
    def prepare_dur_data(self, ori_file_list, output_file_list, label_type="state_align", feature_type=None, unit_size=None, feat_size=None,state_number=5):
        '''
        ...
        for i in range(utt_number):
            self.extract_dur_features(ori_file_list[i], output_file_list[i], label_type, feature_type, unit_size, feat_size, state_number=5)

    def extract_dur_features(self, in_file_name, out_file_name=None, label_type="state_align", feature_type=None, unit_size=None, feat_size=None,state_number=5):
        logger = logging.getLogger("dur")
        # pdb.set_trace()
        if label_type=="phone_align":
            A = self.extract_dur_from_phone_alignment_labels(in_file_name, feature_type, unit_size, feat_size)
        elif label_type=="state_align":
            A = self.extract_dur_from_state_alignment_labels(in_file_name, feature_type, unit_size, feat_size,state_number=5)
        else:
            logger.critical("we don't support %s labels as of now!!" % (label_type))
            sys.exit(1)
    ...
    def extract_dur_from_state_alignment_labels(self, file_name, feature_type, unit_size, feat_size, state_number):
        logger = logging.getLogger("dur")
        ...
        for line in utt_labels:
            ...
            if feature_type == "binary":
                ...
                # current_block_array = numpy.zeros((frame_number, 1))
                if unit_size == "state":
                    current_dur_array[current_index%state_number] = frame_number
    ...
    def load_labels_with_state_alignment(self, file_name,state_number=5):
...
#                print   line
                if temp_list[0] == 'CQS':
                    ...
                    if 'Utterance' in question_key and '$' in processed_question:
                               ## added by ZHANGGUANGYAN 29 SEP 2018
