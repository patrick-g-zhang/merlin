...
def post_filter(mgc_file_in, mgc_file_out, mgc_dim, pf_coef, fw_coef, co_coef, fl_coef, gen_dir, cfg):
    ...
    # pdb.set_trace()
    run_process('{line} | {x2x} +af > {weight}'.format(line=line,
                                                       x2x=SPTK['X2X'], weight=os.path.join(gen_dir, 'weight')))

    run_process('{freqt} -m {order} -a {fw} -M {co} -A 0 < {mgc} | {c2acr} -m {co} -M 0 -l {fl} > {base_r0}'.format(
        freqt=SPTK['FREQT'], order=mgc_dim - 1, fw=fw_coef, co=co_coef, mgc=mgc_file_in, c2acr=SPTK['C2ACR'], fl=fl_coef, base_r0=mgc_file_in + '_r0'))
...
#    NND      = cfg.NND
    ...
    if isinstance(cfg.fw_alpha, str):
        if cfg.fw_alpha == 'Bark':
            ...
            raise ValueError('cfg.fw_alpha=' + cfg.fw_alpha +
    ...
    for filename in file_id_list:
     ...
     #   pdb.set_trace()
        ...
        if cfg.do_post_filtering:
            ...
            # pdb.set_trace()
            post_filter(files['mgc'], mgc_file_name, cfg.mgc_dim,
                        pf_coef, fw_coef, co_coef, fl_coef, gen_dir, cfg)
        ...
        if cfg.vocoder_type == 'STRAIGHT':
            ...
            run_process('{sopr} -magic -1.0E+10 -EXP -MAGIC 0.0 {lf0} > {f0}'.format(
                sopr=SPTK['SOPR'], lf0=files['lf0'], f0=files['f0']))
            run_process(
                '{x2x} +fa {f0} > {f0a}'.format(x2x=SPTK['X2X'], f0=files['f0'], f0a=files['f0'] + '.a'))
            ...
            run_process('{sopr} -magic -1.0E+10 -EXP -MAGIC 0.0 {lf0} | {x2x} +fd > {f0}'.format(
                sopr=SPTK['SOPR'], lf0=files['lf0'], x2x=SPTK['X2X'], f0=files['f0']))

            run_process('{sopr} -c 0 {bap} | {x2x} +fd > {ap}'.format(
                sopr=SPTK['SOPR'], bap=files['bap'], x2x=SPTK['X2X'], ap=files['ap']))
            ...
            run_process(
                'rm -f {ap} {sp} {f0}'.format(ap=files['ap'], sp=files['sp'], f0=files['f0']))
        elif cfg.vocoder_type == 'WORLD_PY':
            logging.info(
                "generate speech with py world, sampling rate is {0}".format(cfg.sr))
            # pdb.set_trace()
            lf0_file = files['lf0']
            ...
            ap = pyworld.decode_aperiodicity(bap.astype(
                np.float64).reshape(-1, bap_dim), cfg.sr, cfg.fl)
